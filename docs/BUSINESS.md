...
Why this price, and why a subscription (not rev-share): Asili replaces the unaffordable "first hire" — a part-time VA runs **$800–2,000/mo** — so **$99 for an always-on operations team is a fraction of that**, while Asili takes **0% of the seller's sales**. The flat fee is predictable for a thin-margin seller and aligns price with *operations relief*, not their revenue. Routine, high-volume turns run on the cheaper model tier, so **gross margin expands as a seller scales** (instrumented live at `/api/metrics`).
