...
- **Model tiering + cost meter — wired.** Routine, high-volume turns (Messaging, Pricing agents) run on `gemini_model_routine` (a cheaper/faster tier) while the Operations Manager runs on `gemini_model_complex`; `tools/cost.py` prices real token usage per tier per seller (from ADK `usage_metadata` when present), so the cost-per-seller curve is populated from production and bends down as routine volume grows. The baseline stays on the default model so the Trust Scorecard comparison is fair.
