...
- **Approval gate — wired (capability-based, fail-closed).** No agent is given a send tool, so it physically cannot message a customer; the only outbound path is `POST /api/approve` after the seller taps Approve. The gate **fails closed**: with no approval callback registered a draft stays `PENDING` rather than auto-sending — auto-approve only happens behind an explicit `set_auto_approve(True)` opt-in (the CLI demo). Under the running app the callback is registered and every draft waits for the seller.
...
In short: the MongoDB-MCP/Atlas grounding **read** path, the deterministic pricing engine, the fail-closed capability-based approval gate, the durable conversation/draft write path, the Trust Scorecard endpoint, and the no-tools baseline are running as drawn; the only persistence still in-process is the decision log + `eval_runs` history.
