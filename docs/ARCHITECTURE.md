...
The diagram above is the architecture the system runs. Here is precisely what is wired in the current codebase versus the one increment still staged:
...
- **Approval gate — wired (capability-based).** No agent is given a send tool, so it physically cannot message a customer; the only outbound path is `POST /api/approve` after the seller taps Approve. (In a bare demo with no approval callback registered, `send_for_approval` auto-approves; under the running app the callback is registered and the gate holds.)
